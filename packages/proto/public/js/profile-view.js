...
function renderSlots(json) {
    ...
    const slot = ([key, value]) => {
      ...
      switch (type) {
        case "array":
          return `<div class="profile-info" slot="${key}">
                    <span class="label"></span>
                    <ul>${value.map(s => `<li>${s}</li>`).join("")}</ul>
                  </div>`;
  
        case "avatar":
          return `<profile-avatar slot="${key}" color="${json.color}" src="${value}"></profile-avatar>`;
  
        default:
          // Rendering label and value with visual space
          return `<div class="profile-info" slot="${key}">
                    <span class="label"></span>
